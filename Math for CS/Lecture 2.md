...
3. **시간 기반 이상의 응용**
  ...
  - **GPS 감도:** GPS 위치 오차 전파([[AI_deep_dive/GPS location error propagation|GPS_location_error_propagation]])를 미분을 사용하여 분석할 수 있습니다.
...
5. **불연속성의 유형**
  - **점프 불연속성([[jump discontinuity|jump discontinuity]]):** 좌측과 우측 극한은 존재하지만 값이 다릅니다.
  - **제거 가능한 불연속성(removable discontinuity):** 극한은 존재하지만 함수값과 다릅니다.
  - **무한불연속성(infinit discontinuity):** 함수가 $\pm \infty$로 발산합니다.
  - 기타 (못생긴) 불연속성(other (ugly) discontinuity): 함수가 극한을 가지지 않습니다.
