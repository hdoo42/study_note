...
일반적으로 모델의 **손실 함수**([[Loss_Function 1|Loss function]])에 다음과 같은 **제약항(penalty term)** 을 추가하여 구현한다.
...
2. **과적합([[Overfitting 1|Overfitting]]) 방지**  
...
3. **해석 가능성([[Interpretability 1|Interpretability]]) 향상**  
...
#### **3.1 선형 회귀([[linear_regression 1|Linear Regression]]) 예시**
- 선형 회귀에서 L1 정규화를 적용한 것을 **라소 회귀([[Lasso_Regression 1|Lasso Regression]])**라고 한다.  
- 모델 식:  
  $$
...
#### **3.2 딥러닝에서의 활용**
- 딥러닝에서는 일반적으로 L2 정규화([[weight_decay 1|Weight Decay]])가 더 자주 쓰이지만, L1 정규화를 사용하면 **가중치를 희소화**할 수 있다.  
